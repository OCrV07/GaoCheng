...
{
	if (prompt[0] == '\0') {
		cout << "2^" << setw(4) << (int)(char)d[0] << " : 0.";
		int n = -(char)d[0]; // 确认输出的位数
		for (int j = 1; j <= n; j++) {
			cout << (char)(d[j] + '0'); // 按位输出
		}
		cout << endl;
	}
	else {
	...
	for (int k = 0; k < len; k++) {
		if (pure_decimal[k] == '1') {
			...
			for (int j = N_NUM; j >= 1; j--) {
				int val = sum[j] + power_table[pow_idx][j] + carry;
				sum[j] = val % 10;
				carry = val / 10;
			}
		}
...
void input_pure_decimal(char pure_decimal[])
{
	...
	do {
		valid = true;
		cout << "请输入一个二进制纯小数，小数点开头，小数点后不超过112位(例：."
				"101101)"
			 << endl;
