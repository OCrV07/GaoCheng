...
const int HDC_Base_Width =
	8; // 基本宽度w，单位像素点，人为保证是 HDC_Step_X 的倍数（柱子的宽度=w，每个盘子的宽度分别是3w~21w）
const int HDC_Base_High = 16; // 基本高度h，单位像素点，人为保证是 HDC_Step_Y 的倍数（底座的高度=h，每个盘子的高度=h）
const int HDC_Top_Y = 20;	  // 设置顶部Y坐标，盘子移动时，到该坐标即停止，人为保证 （HDC_Start_Y - HDC_Top_Y）是
							  // HDC_Step_Y 的倍数（最小值为0，表示移动到顶部）
...
const int HDC_COLOR[MAX_LAYER + 2] = {
	...
	RGB(19, 161, 14),	RGB(58, 150, 221),	RGB(197, 15, 31),  RGB(136, 23, 152), RGB(193, 156, 0),
	RGB(204, 204, 204), RGB(118, 118, 118), RGB(59, 120, 255), RGB(22, 198, 12), // 10#盘(最大的盘子)的颜色
